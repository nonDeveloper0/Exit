...
**전화 연출 초기화(관리자):** 관리자 화면 `수신전화 연출` 카드의 `전화 연출 초기화` 버튼을 누르면, 공기계에 남아 있는 `통화내용 다시 듣기` 버튼이 사라지고 대기 화면으로 되돌아간다. 폰 상태는 공기계 브라우저에만 저장되므로, 관리자는 이 버튼으로 Supabase 브로드캐스트(`call_reset`)를 보내 원격으로 초기화한다. 진행 중이던 수신전화 마커도 함께 해제된다. (다음 조에게 다시 전화를 걸기 전에 눌러 깨끗한 상태로 만드는 용도.)
## 1-4. 수신전화 벨소리 / 진동 조정
...
사진 보드의 장소 탭을 선택한 뒤 촬영하면 그 장소로 자동 저장됩니다. 업로드에는 캡션만 입력하며, 라이트박스 → **정보 수정**에서 증거 설명(20자 메모)과 장소를 수정할 수 있습니다. 저장하면 같은 조(짝 조 포함)에 실시간 반영됩니다. 사진은 **조별 최대 30장**이며, 짝 조와 한도를 공유하지 않습니다. 예를 들어 1조와 4조가 짝 조이면 각 30장씩, 총 60장까지 등록할 수 있습니다.

잘못 찍은 사진은 라이트박스 → **사진 삭제**로 조장이 직접 지울 수 있습니다(확인 단계 있음). 삭제는 **자기 조 사진만** 가능하고 짝 조 사진은 지울 수 없으며, Storage 파일과 `photo_evidence` 행이 함께 영구 삭제됩니다(되돌릴 수 없음). 삭제하면 조별 30장 한도에서도 그만큼 슬롯이 비워집니다. 삭제 로직은 `usePhotoEvidence`의 `deletePhoto`입니다.

사진 업로드와 QR 스캐너는 조장만 사용할 수 있습니다. 용의자별 **수사 노트**는 `열기/접기` 버튼으로 펼치며, 조 전체에 실시간 공유됩니다. 본인이 남긴 메모만 삭제할 수 있습니다.
...
현재 최종 투표는 **조별 1회 제출**입니다.
제출 기록은 서버(Supabase `final_votes` 테이블)에 조(`pair_id`)당 한 행으로 저장됩니다(선택 용의자 + 서술형 근거 + 제출자 이름). 기기를 바꾸거나 새로고침해도 유지되며, 관리자 초기화로 행을 지우면 Realtime으로 각 기기에 즉시 반영됩니다. (조회/저장 로직: `src/lib/useFinalVote.ts`)

제출 시 **구글폼**으로도 전송됩니다(범인 선택 + 서술형 근거). 구글폼 응답이 채점의 원본이고, `final_votes`는 "이미 제출함" 상태 표시용입니다.

**초기화:** `/admin`의 "최종추리 제출 초기화"(개별) 또는 "전체 조 초기화"가 `final_votes` 행을 삭제합니다.
...
- 단, `public/`의 정적 파일이라 **URL을 아는 사람은 열 수 있음**(은닉이지 차단 아님).
  실제 잠금은 화면 안의 공통 비밀번호 입력이 담당(노트북 기준).
...
잠금 방식: **공통 암호 입력만**. (조 번호 선택은 제거됨 — 노트북에서는 조별 증거 수집을 하지 않습니다.)
바탕화면에는 **지문감식 결과보고서.pdf** 1개만 있으며, 열면 열람 완료 토스트만 뜹니다(어느 조 보관함에도 저장하지 않음).
(예전의 "MOSES+조번호" 개별 암호 방식, "조 선택 + 공통 암호" 방식 모두 폐기.)
...
> ⚠️ **아이패드 화면(`ipad.html`)과 노트북(`laptop.html`) 모두 조별 수집 연동 없이 연출용 화면**입니다.
> 조별 증거 수집이 다시 필요하면 조 번호 입력 + `team_evidence_items` 저장 로직을 되살려야 합니다(이전 버전 git 이력 참고).
