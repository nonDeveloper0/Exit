...
사진 점검은 관리자(`/admin`)의 **사진 점검**에서 합니다. 상단 조별 집계에서 각 조의 유효 사진·등록·제외 수를 한눈에 확인하고, 조별 필터로 사진을 찾습니다. 스팸 사진은 `제외`를 누릅니다. 제외 사진은 삭제되지 않으며 참가자 보드에는 `제외됨`으로 표시됩니다. 제외 즉시 증거함의 유효 사진 수·장소별 수·업로드 한도와 사진 랭킹에서 빠집니다. 잘못 제외한 사진은 같은 자리에서 `복원`하면 즉시 다시 반영됩니다.
