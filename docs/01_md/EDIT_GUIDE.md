...
- `public/screen/phone2.html` → 채소장 휴대폰 화면 (홈 화면 + 전화 + 메시지 + 설정)

**여는 방법 (운영자 기기에서 URL 직접 입력):**
```
https://(배포주소)/screen/ipad.html
https://(배포주소)/screen/laptop.html
https://(배포주소)/screen/phone2.html
```
...
- **뒤로가기 방지:** 세 화면 모두 `<script>` 맨 위에 히스토리를 계속 되채우는 코드가 있어, 모바일 브라우저의
  뒤로가기 버튼/스와이프 제스처를 눌러도 화면을 벗어나지 못하고 그대로 남아 있는다(`history.pushState` +
  `popstate` 리스너). 참가자가 실수로 기기 화면에서 이탈해 몰입이 깨지는 걸 막기 위함.
...
- 홈 화면: 전화 · 메시지 · 설정만 동작. 카메라·갤러리·캘린더는 장식용 아이콘(클릭 무반응)
...
- **설정 앱**: `세션 종료` 버튼으로 다음 조를 위해 초기화(잠금화면으로 복귀). 90초 무입력 시 자동으로도 초기화됨
...
**사용(운영자):** `/admin` → **제한 시간 타이머** 섹션

- 분을 직접 입력하거나 `5 / 10 / 15 / 30` 프리셋 버튼으로 고른 뒤 **`타이머 시작`** 을 누른다.
- 시작하면 **모든 참가자 기기 상단에 카운트다운 배너**가 뜨고, 1분 이하로 남으면 빨간색으로 바뀐다.
- 0이 되면 각 기기에서 **경보음이 약 4초간 울린다**(Android는 진동도). 배너는 `시간 종료`로 바뀐다.
- **`종료`** 를 누르면 카운트다운/배너가 즉시 사라진다. `타이머 재시작`은 새 시간으로 다시 시작한다.

**제약:** 경보음은 수신전화 벨소리와 같은 웹 자동재생 제약을 받는다 — 참가자가 앱을 한 번이라도
터치했고 앱이 화면에 떠 있어야 소리가 난다(백그라운드/화면잠금이면 무음). iPhone은 진동이 항상 무시된다.

**코드 조정:**
- 경보 음색/길이: `src/lib/ringtone.ts`의 `playAlarm`(주파수 `988`Hz, 반복 `count >= 4`).
- 배너 모양/색: `src/components/TimerOverlay.tsx`.
- 저장은 스키마 변경 없이 `team_evidence_items` 전역 마커 1행(`pair_id=__global`, `type='timer'`)의
  `created_at`에 **종료 시각**을 넣어 처리한다. 모든 기기가 이 값을 읽어 동일하게 카운트다운한다.
