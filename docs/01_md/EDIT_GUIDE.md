...
## 0. 첫 입장 화면 조 번호 (드롭다운 목록)

**파일:** `src/app/page.tsx`

첫 입장 화면의 조 번호는 **직접 입력이 아니라 드롭다운 선택**입니다. 목록은 파일 상단 상수 하나로 정해집니다.
...
- **조를 늘리려면** 배열에 값을 추가합니다. 예: 8조까지 → `["1","2","3","4","5","6","7","8"]`
- 선택한 값이 그대로 조 ID(`pair_id`)로 저장되므로, 관리자 화면의 **조장 지정·짝 조 매핑에 쓰는 조 번호와 문자열이 정확히 같아야** 합니다(`1`은 되지만 `01`, `1조`는 안 됨).
- 목록에 없는 조 번호는 선택할 수 없고, 예전에 저장된 값이 목록에 없으면 선택이 비어 있는 상태로 시작합니다(다시 선택해야 입장 가능).
- 한 번 입장한 기기는 조 번호와 이름이 잠깁니다. 잘못 입장한 경우에만 화면의 `조 또는 이름을 변경해야 하나요?`를 열고 Staff 확인 후 PIN을 입력해 변경합니다.
- 운영자 PIN 기본값은 `9999`입니다. 배포 환경변수 `NEXT_PUBLIC_TEAM_CHANGE_PIN`을 설정하면 그 값으로 바뀝니다. 이 PIN은 참가자 실수 방지용이며, 브라우저에서 확인 가능한 값이므로 보안 권한 체계로 사용하면 안 됩니다.

---

## 0-1. 관리자 권한과 화면 정리

`/admin`의 메뉴는 접기/펼치기 방식으로 정리되어 있습니다. 필요한 항목의 `열기`를 눌러 사용하세요.

- **게임 진행:** 최종 투표, 엔딩, 수신전화 연출
- **권한 · 조 구성:** 스탭 권한, 조장 지정, 짝 조 매핑
- **사진 점검:** 조별 사진 검토 및 제외 처리
- **초기화 · 전체 삭제:** 진행 기록, 조별 증거 초기화와 전체 삭제. 전체 조 초기화는 기록이 없어도 항상 실행할 수 있습니다.

스탭 권한은 `권한 · 조 구성` → `스탭 권한`의 이름 입력칸에 적고 **추가**하면 됩니다. 추가된 이름은 조 번호와 무관하게 조장 권한을 얻으며, 목록의 **제거**로 취소할 수 있습니다. 기본 스탭 명단은 코드에 고정되어 있으므로 이 목록에서는 제거되지 않습니다.
...
잘못 찍은 사진은 라이트박스 → **사진 삭제**로 조장이 직접 지울 수 있습니다(확인 단계 있음). 삭제·정보 수정은 **조장이 볼 수 있는 사진(자기 조 + 짝 조)** 모두에 적용되며, 삭제 시 Storage 파일과 `photo_evidence` 행이 함께 영구 삭제됩니다(되돌릴 수 없음). 짝 조와 무관한 다른 조의 사진은 애초에 조회되지 않습니다. 삭제하면 해당 조의 30장 한도에서 그만큼 슬롯이 비워집니다. 삭제 로직은 `usePhotoEvidence`의 `deletePhoto`이고, 권한 게이팅은 `src/app/evidence/page.tsx`에서 `isLeader`로만 처리합니다.
...
사진 점검은 관리자(`/admin`)의 **사진 점검**에서 합니다. 상단 조별 집계에서 각 조의 유효 사진·등록·제외 수를 한눈에 확인하고, 조별 필터로 사진을 찾습니다. 스팸 사진은 `제외`를 누릅니다. 제외 사진은 삭제되지 않으며 참가자 보드에는 `제외됨`으로 표시됩니다. 제외 즉시 증거함의 유효 사진 수·장소별 수와 사진 랭킹에서 빠집니다. 단, 제외 사진도 등록 사진이므로 조당 30장 업로드 한도에는 계속 포함됩니다. 잘못 제외한 사진은 같은 자리에서 `복원`하면 즉시 다시 반영됩니다.
...
## 7-1. 짝 조 팀 이름 배지 (수사본부 우측 상단)

관리자 `/admin` → **조 매핑**에서 두 조를 팀 이름(예: `분홍`)과 함께 매핑하면, 그 페어 참가자의 수사본부 우측 상단에 팀 배지가 표시됩니다.

- **순번 자동 부여:** 짝 조 중 **조 번호가 작은 쪽이 `분홍1`, 큰 쪽이 `분홍2`**로 표시됩니다(예: 3·6조 → 3조=분홍1, 6조=분홍2). 관리자는 이름(`분홍`)만 입력하면 되고 숫자는 코드가 자동으로 붙입니다.
- 순번 계산: `src/lib/pairTeam.ts`의 `pairTeamIndex()` (조 번호 숫자 비교, 비숫자면 사전순).
- 표시 문자열: `formatPairTeamName(name, index)` — index를 주면 `분홍1`, 없으면 기존 `분홍 팀` 형식.
- 배지 색상: `getPairTeamTone()` — 이름에 `노랑`/`파랑`/`분홍`이 들어가면 각 색, 그 외 앰버.
- 표시 데이터 훅: `src/lib/usePairTeamName.ts` (`{ name, index }` 반환).

---

## 7-2. 진행 시간표 이미지 (수사본부 최하단)

수사본부 탭 맨 아래에 `진행 시간표` 카드로 이미지를 표시합니다.

- **이미지 교체:** `public/타임테이블.png` 파일을 같은 이름으로 덮어쓰면 됩니다(코드 수정 불필요).
- 파일명·제목을 바꾸려면 `src/app/home/page.tsx`의 `{/* Timetable */}` 블록에서 `src="/타임테이블.png"`와 `진행 시간표` 문구를 수정합니다.
- 이미지는 카드 폭에 맞춰 자동 축소됩니다(`w-full`).
...
**암호 잠금 있음** — `/screen/phone2`로 열면 iOS 스타일 암호 잠금화면이 먼저 뜨고, 4자리 암호 `0625`를 입력해야 홈 화면으로 들어간다.
- **실제 서빙 파일은 `public/screen/dongguri_phone_room.html`**다(`/screen/phone2` → 이 파일, `next.config.ts` rewrite). `public/screen/back/phone2.html`은 옛 백업본이며 라이브가 아니다. (아래 홈/전화/메시지 세부 설명 일부는 이 백업본 기준이라 라이브와 다를 수 있음.)
- **암호 변경:** `<script>` 안의 `const LOCK_CODE = "0625";` 값을 원하는 4자리로 수정. 자릿수를 바꾸려면 `lockPress`의 `length >= 4`·`length === 4`와 `#lockDots`의 `.dot` 개수도 함께 맞춘다.
...
### 13-1. 초기화 카테고리 2차 PIN

스탭이 관리자 화면을 만지다 실수로 초기화하지 않도록, `초기화 · 전체 삭제` 섹션은 **관리자 PIN과 별개인 2차 PIN**을 입력해야 열립니다. 한 번 해제하면 해당 화면을 새로고침하기 전까지는 계속 열려 있습니다.

PIN 값은 `src/app/admin/page.tsx` 상단에 있습니다.

```ts
const ADMIN_PASSWORD = "0000";  // 관리자 화면 진입 PIN
const RESET_PASSWORD = "9999";  // 초기화 카테고리 2차 PIN
```

- 숫자 4자리로 바꾸면 됩니다. 두 값을 다르게 두어야 2차 인증 의미가 있습니다.
