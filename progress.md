...
- [x] 채소장(B, QR `w3n5k7`) 심문권 퀴즈 `question`을 `🤮` 이모티콘에서 `"화학용액 조제 및 배합 공정"`으로 되돌렸다.
  - 변경: `src/lib/data.ts`, `progress.md`
...
- [x] 관리자 `사진 점검` 상단에 1~6조별 요약 카드를 추가했다. 각 카드는 유효 사진 수, 전체 등록 수, 제외 수를 표시해 조별 집계를 한눈에 확인할 수 있다.
  ...
  - 검증: 사진 필터 테스트 3개 통과, 변경 파일 ESLint 통과, `npm.cmd run build`, `git diff --check` 통과.
...
- [x] `CLAUDE.md` 개요의 조 개수를 실제 운영 기준인 6개(1~6조)로 수정했다. 기존 표기는 8개였다. 인원(80명)과 시간(120분)은 그대로 두었다.
  - 변경: `CLAUDE.md`, `progress.md`
...
- [x] 고정 스탭 조장 명단에 정재덕을 추가했다. 해당 이름으로 입장하면 조 지정과 무관하게 조장 권한을 가진다.
...
- [x] 수사본부 사건 개요의 "형사는 이 다섯 사람을 용의자로 특정한다." 문단 아래에 `사망추정시간 20:40-20:50` 한 줄을 앰버색 강조로 추가했다.
  - 변경: `src/app/home/page.tsx`, `progress.md`
  - 검증: `src/app/home/page.tsx` eslint 통과.
...
- [x] 최종 추리 근거를 필수 입력으로 변경했다. 공백만 입력한 경우에도 제출 버튼이 비활성화되며, 제출 함수에서도 빈 근거를 차단한다. 예시는 "A는 ~~이어서 범인으로 보기 어렵습니다. C는 ~~이므로 범인으로 추리합니다." 형식으로 적용했다.
  - 변경: `src/app/vote/page.tsx`, `progress.md`
...
- [x] evidence(증거 보관함) 페이지에 참가자용 사진 삭제 기능 추가. 라이트박스에 삭제 버튼(조장 전용·자기 조 사진만·확인 단계). 삭제 시 Storage 파일 + `photo_evidence` row 제거.
  ...
  - `docs/01_md/EDIT_GUIDE.md`: 1-6 사진 증거 섹션에 삭제 안내 추가.
